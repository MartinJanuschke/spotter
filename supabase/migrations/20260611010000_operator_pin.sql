-- Operator PIN moves from the client environment into the database: stored
-- bcrypt-hashed in app_settings, verified through an RPC so the hash never
-- leaves the server. Change it with:
--   update public.app_settings
--   set value = extensions.crypt('NEW_PIN', extensions.gen_salt('bf'))
--   where key = 'operator_pin';
...
values ('operator_pin', extensions.crypt('2468', extensions.gen_salt('bf')));
