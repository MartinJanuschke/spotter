---
name: ux-heuristics
description: 'Evaluate and improve interface usability using heuristic analysis (Krug + Nielsen). Use this skill whenever the user asks about usability, UX quality, or interface clarity — including: "usability audit", "UX review", "heuristic evaluation", "is this good UX?", "users are confused", "users keep dropping off", "form usability", "navigation problems", "is this clear?", "does this make sense to users?", "how do I improve this screen?", "review my UI", "what UX issues do you see?", or any request to evaluate, critique, or improve a user interface. Also use it when the user shares a component, screen, or flow and wants feedback — even if they don''t use the word "usability". For visual design fixes see refactoring-ui; for conversion-focused audits see cro-methodology.'
license: MIT
metadata:
  author: wondelai
  version: "1.2.2"
...
Use this framework whenever evaluating or improving any UI — a screen, component, flow, copy, or navigation structure.

**Every audit must produce:**

1. **Score: X/10** — Rate the interface against the principles below. Be honest and calibrated: a typical first-pass interface scores 5–7. Reserve 9–10 for genuinely excellent work. State the score prominently.

2. **Issues list** — For each problem found, include:
   - Which heuristic or Krug law is violated (name it explicitly)
   - Severity rating: 0 (ignore) → 1 (cosmetic) → 2 (minor) → 3 (major) → 4 (catastrophic)
   - A one-line description of the problem
   - A concrete fix — specific, actionable, not generic advice

3. **Priority order** — Lead with severity 3–4 issues. Severity 1–2 issues come after. This ensures the user knows what to fix first.

**Two modes — adapt to context:**

- **Audit mode** (user shares existing UI): Diagnose what's broken. Use the Quick Diagnostic and Common Mistakes tables to spot issues fast. Apply both Krug's laws and Nielsen's heuristics — violations often cluster.
- **Design guidance mode** (user is building something new): Apply the framework proactively. Recommend patterns that prevent problems rather than just flagging them.

**Be specific, not generic.** "Add a loading indicator" is weak. "The form submit button gives no feedback — add a spinner and disable it on click to prevent double-submission (Nielsen #1: Visibility of System Status)" is strong. Name the heuristic, name the fix.

**Use the reference files when going deep:**
- Complex navigation issues → read `references/krug-principles.md`
- Detailed heuristic breakdowns → read `references/nielsen-heuristics.md`
- Conflicting heuristics → read `references/heuristic-conflicts.md`
- Accessibility concerns → read `references/wcag-checklist.md`
